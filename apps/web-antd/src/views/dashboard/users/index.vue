...
import { Button, Card, Form, Input, message, Modal, Space, Table } from 'ant-design-vue';
...
const columns = [
  ...
  { title: 'Telegram ID', dataIndex: 'tg_user_id', key: 'tg_user_id', width: 180 },
  { title: '展示名', dataIndex: 'display_name', key: 'display_name', width: 180 },
  { title: '用户名', dataIndex: 'username_label', key: 'username_label', width: 260 },
  { title: '折扣', dataIndex: 'cloud_discount_rate', key: 'cloud_discount_rate', width: 120 },
  { title: 'USDT 余额', dataIndex: 'balance', key: 'balance', width: 140 },
  { title: 'TRX 余额', dataIndex: 'balance_trx', key: 'balance_trx', width: 140 },
...
<template>
  <Page description="读取 Django 用户数据，支持用户名、折扣与余额修改" title="用户列表">
    <Card>
      <template #title>
        <Space>
          <span>用户数据</span>
      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'display_name'">
            <div class="font-medium text-gray-900">{{ record.display_name || '-' }}</div>
            <div class="text-xs text-gray-400">{{ record.first_name || '无昵称' }}</div>
          </template>
          ...
          <template v-else-if="column.key === 'actions'">
            <Space>
              <Button type="link" @click="openBalanceDetail(record as DashboardUserItem)">余额明细</Button>
              <Button type="link" @click="openEdit(record as DashboardUserItem)">编辑</Button>
            </Space>
          </template>
        </template>
      </Table>
    ...
    <Modal v-model:open="editOpen" :confirm-loading="saving" title="编辑余额/折扣" @ok="submitEdit">
      <Form layout="vertical">
        <Form.Item label="用户">
          <Input :value="currentRow?.display_name || ''" disabled />
        </Form.Item>
        <Form.Item label="USDT 余额">
          <Input v-model:value="formState.balance" placeholder="请输入 USDT 余额" />
        </Form.Item>
        <Form.Item label="TRX 余额">
          <Input v-model:value="formState.balance_trx" placeholder="请输入 TRX 余额" />
        </Form.Item>
        <Form.Item label="折扣(%)">
          <Input v-model:value="formState.cloud_discount_rate" placeholder="请输入折扣，100 表示无折扣" />
        </Form.Item>
      </Form>
    </Modal>
  </Page>
