...
async function loadData() {
  loading.value = true;
  try {
    items.value = await getDashboardCloudIpLogsApi({ keyword: keyword.value.trim(), log_type: 'server' });
  } finally {
    loading.value = false;
  }
}
...
<template>
  <Page description="记录服务器创建、续费、删除等关键过程，支持关键字搜索" title="服务器日志">
    <Card>
      <template #title>
        <Space>
          <span>服务器日志</span>
          <Input.Search v-model:value="keyword" allow-clear enter-button="搜索" placeholder="搜索订单号 / 用户 / 服务器 / IP / 说明" style="width: 360px" @search="loadData" />
          <Button size="small" @click="resetSearch">重置</Button>
          <Button size="small" @click="loadData">刷新</Button>
        </Space>
      </template>
      <Table :columns="columns" :data-source="items" :loading="loading" row-key="id" :pagination="{ pageSize: 20 }" :scroll="{ x: 1700 }">
