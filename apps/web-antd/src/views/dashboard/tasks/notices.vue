...
const noticeBatchColumns: TableColumnsType<DashboardNoticeUserSummaryItem> = [
  ...
  { title: '通知渠道', dataIndex: 'notice_channel_label', key: 'notice_channel_label', width: 210 },
  { title: 'IP数量', dataIndex: 'ip_count', key: 'ip_count', width: 90 },
  { title: 'IP列表', dataIndex: 'ips', key: 'ips', width: 340 },
  { title: '通知文案', dataIndex: 'notice_text_preview', key: 'notice_text_preview', width: 460 },
  { title: '通知时间', dataIndex: 'next_notice_at', key: 'next_notice_at', width: 180 },
  { title: '失败重试', dataIndex: 'failed_retry_count', key: 'failed_retry_count', width: 100 },
...
const historyColumns: TableColumnsType<DashboardNoticePlanHistoryItem> = [
  ...
  { title: '通知文案', dataIndex: 'notice_text_preview', key: 'notice_text_preview', width: 500 },
  { title: '重试说明', dataIndex: 'retry_label', key: 'retry_label', width: 300 },
  { title: '操作', key: 'actions', width: 100, fixed: 'right' },
];

const dueItems = computed(() => detail.value?.due_items || []);
const dueBatchItems = computed(() => detail.value?.due_user_summary_items || []);
const futureBatchItems = computed(() => detail.value?.future_user_summary_items || []);
...
<template>
  <Page description="按用户和通知类型整合通知计划；同一种通知只生成一条合并文案" title="通知计划">
      ...
      <Card :loading="loading">
        ...
        <Descriptions bordered :column="2" size="small">
          ...
          <Descriptions.Item label="最近24小时送达">
            <Tag color="success">{{ detail?.recent_success_user_count ?? 0 }} 用户</Tag>
            / {{ detail?.recent_success_count ?? 0 }} 条
          </Descriptions.Item>
          <Descriptions.Item label="最近24小时失败">
            <Tag color="error">{{ detail?.recent_failure_user_count ?? 0 }} 用户</Tag>
          ...
          <Descriptions.Item label="未来计划">
            <Tag color="processing">{{ detail?.future_user_count ?? 0 }} 种通知</Tag>
        ...
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'notice_type_label'">
              <Tag :color="noticeColor((record as DashboardNoticeUserSummaryItem).notice_type)">
                {{ (record as DashboardNoticeUserSummaryItem).notice_type_label }}
              </Tag>
            </template>
            <template v-else-if="column.key === 'notice_channel_label'">
              <Tag :color="channelColor((record as DashboardNoticeUserSummaryItem).notice_channel)">
                {{ (record as DashboardNoticeUserSummaryItem).notice_channel_label || '-' }}
              </Tag>
            </template>
            <template v-else-if="column.key === 'ips'">
              <TypographyParagraph :ellipsis="{ rows: 2, tooltip: (record as DashboardNoticeUserSummaryItem).ips.join(' / ') }" class="mb-0 break-all text-sm leading-6">
                {{ (record as DashboardNoticeUserSummaryItem).ips.join(' / ') || '-' }}
              </TypographyParagraph>
            </template>
            <template v-else-if="column.key === 'notice_text_preview'">
              <TypographyParagraph :ellipsis="{ rows: 2, tooltip: (record as DashboardNoticeUserSummaryItem).notice_text_preview }" class="mb-0 break-all text-sm leading-6">
                {{ (record as DashboardNoticeUserSummaryItem).notice_text_preview || '-' }}
              </TypographyParagraph>
            </template>
            <template v-else-if="column.key === 'next_notice_at'">
              {{ fmtTime((record as DashboardNoticeUserSummaryItem).next_notice_at) }}
            </template>
            <template v-else-if="column.key === 'failed_retry_count'">
              <Tag :color="(record as DashboardNoticeUserSummaryItem).failed_retry_count > 0 ? 'error' : 'default'">
                {{ (record as DashboardNoticeUserSummaryItem).failed_retry_count }}
              </Tag>
            </template>
            <template v-else-if="column.key === 'actions'">
              <Button type="link" size="small" @click="openOrder((record as DashboardNoticeUserSummaryItem).related_path)">
                订单详情
              </Button>
            </template>
          </template>
      ...
      <Card title="历史通知（按实际发送批次整合）">
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'ips'">
              <TypographyParagraph :ellipsis="{ rows: 2, tooltip: (record as DashboardNoticePlanHistoryItem).ips.join(' / ') }" class="mb-0 break-all text-sm leading-6">
                {{ (record as DashboardNoticePlanHistoryItem).ips.join(' / ') || (record as DashboardNoticePlanHistoryItem).ip || '-' }}
              </TypographyParagraph>
            </template>
            <template v-else-if="column.key === 'notice_text_preview' || column.key === 'retry_label'">
              <TypographyParagraph :ellipsis="{ rows: 2, tooltip: String((record as any)[column.key] || '-') }" class="mb-0 break-all text-sm leading-6">
