...
const userSummaryColumns: TableColumnsType<DashboardNoticeUserSummaryItem> = [
  ...
  { title: '通知数', dataIndex: 'notice_count', key: 'notice_count', width: 90 },
  { title: '通知类型', dataIndex: 'notice_type_labels', key: 'notice_type_labels', width: 240 },
  { title: 'IP列表', dataIndex: 'ips', key: 'ips', width: 360 },
  { title: '下次通知', dataIndex: 'next_notice_at', key: 'next_notice_at', width: 180 },
...
const dueUserSummaryItems = computed(() => detail.value?.due_user_summary_items || []);
const futurePlanItems = computed(() => detail.value?.future_plan_items || []);
const futureUserSummaryItems = computed(() => detail.value?.future_user_summary_items || []);
...
function userSummaryRowKey(record: DashboardNoticeUserSummaryItem) {
  return record.id;
}
...
<template>
  <Page
    description="展示两天内待通知、未来通知计划、历史通知，并标明状态、渠道和失败重试规则"
    title="通知计划"
  >
...
{{
          ...
          <Descriptions.Item label="两天内待通知">
            <Tag color="warning">{{ detail?.due_user_count ?? 0 }} 用户</Tag>
            / {{ detail?.due_count ?? 0 }} 个 IP 通知项
          </Descriptions.Item>
          <Descriptions.Item label="未来计划">
            <Tag color="processing">{{ detail?.future_user_count ?? 0 }} 用户</Tag>
      ...
      <Card title="待通知用户统计（按用户整合）">
        <Table
          :columns="userSummaryColumns"
          :data-source="dueUserSummaryItems"
          :loading="loading"
          :pagination="false"
          :row-key="userSummaryRowKey"
          :scroll="{ x: 1450 }"
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'notice_channel_label'">
            ...
            <template v-else-if="column.key === 'notice_type_labels'">
              <Space wrap>
                <Tag v-for="label in (record as DashboardNoticeUserSummaryItem).notice_type_labels" :key="label">
                  {{ label }}
                </Tag>
              </Space>
            </template>
      ...
      <Card title="未来通知用户统计（按用户整合）">
        <Table
          :columns="userSummaryColumns"
          :data-source="futureUserSummaryItems"
          :loading="loading"
          :pagination="false"
          :row-key="userSummaryRowKey"
          :scroll="{ x: 1450 }"
      ...
      <Card title="历史通知">
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template
              v-else-if="
                column.key === 'text_preview' || column.key === 'retry_label'
              "
            >
              <TypographyParagraph
                :ellipsis="{
